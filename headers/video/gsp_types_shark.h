    ...
    typedef struct _GSP_REG_T_TAG_
    {
        union _GSP_CFG_TAG //0x0000
        {
            struct _GSP_CFG_MAP
            {
                ...
                __volatile__ uint32_t y2r_opt           :1; // 0 : full YUV;  1:reduce YUV
                __volatile__ uint32_t no_split          :1; // 0: split, split into 2 burst request, 1 : not split, one burst stride pages boarder
